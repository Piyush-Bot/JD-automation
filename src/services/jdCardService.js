...
function buildDeptCard(departments) {
    ...
    const card = {
        ...
        body: [
            ...
            { type: 'TextBlock', text: 'Select a Department:', wrap: true, size: 'Small', weight: 'Bolder', spacing: 'Small' },
...
function buildRoleCard(deptId, deptName, roles) {
    ...
    const card = {
        ...
        body: [
            ...
            { type: 'TextBlock', text: 'Select a Role:', wrap: true, size: 'Small', weight: 'Bolder', spacing: 'Medium' },
...
function buildJdFormConfirmCard(deptName, roleName, originatorName, reviewerName, approverName) {
    const card = {
        ...
        body: [
            ...
            { type: 'TextBlock', text: 'Your selections:', wrap: true, size: 'Small', spacing: 'Small' },
            {
                type: 'FactSet',
                facts: [
...
function buildEditFormCard(editType) {
    ...
    const card = {
        ...
        body: [
            ...
            { type: 'TextBlock', text: 'Label:', wrap: true, size: 'Small', weight: 'Bolder', spacing: 'Medium' },
...
function buildIndentJobMasterCard(rows) {
    const body = [
        ...
        { type: 'TextBlock', text: rows.length === 0 ? 'No records found.' : `${rows.length} record(s) loaded from database.`, wrap: true, size: 'Small', spacing: 'Small', isSubtle: true }
    ];
    ...
    display.forEach((row, index) => {
        body.push({ type: 'TextBlock', text: `Record ${index + 1}`, weight: 'Bolder', size: 'Medium', separator: true, spacing: 'Medium', wrap: true });
        for (const [label, key] of fields) {
            body.push({ type: 'TextBlock', text: `**${label}:** ${formatCell(rowField(row, key))}`, wrap: true, size: 'Small', spacing: 'None' });
        }
    });
